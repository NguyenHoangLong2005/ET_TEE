...
-- ===== Tables needed by backend staff services =====
ALTER TABLE shipments ADD COLUMN IF NOT EXISTS cod_reconciled BOOLEAN NOT NULL DEFAULT FALSE;
...
CREATE TABLE IF NOT EXISTS stock_reservations (
    ...
    product_id UUID NOT NULL REFERENCES product_variants(id),
...
BEGIN
    ...
    -- Variants (need attribute_values for size+color)
    ...
    IF NOT FOUND THEN
        ...
        VALUES(v_var1,v_loc,50,0,10,now());
    END IF;
    PERFORM * FROM inventory WHERE variant_id=v_var2 AND location_id=v_loc;
    IF NOT FOUND THEN
        INSERT INTO inventory(variant_id,location_id,quantity_on_hand,quantity_reserved,reorder_level,updated_at)
        VALUES(v_var2,v_loc,40,0,10,now());
    END IF;
    PERFORM * FROM inventory WHERE variant_id=v_var3 AND location_id=v_loc;
    IF NOT FOUND THEN
        INSERT INTO inventory(variant_id,location_id,quantity_on_hand,quantity_reserved,reorder_level,updated_at)
        VALUES(v_var3,v_loc,30,0,10,now());
    END IF;
    PERFORM * FROM inventory WHERE variant_id=v_var4 AND location_id=v_loc;
    IF NOT FOUND THEN
        INSERT INTO inventory(variant_id,location_id,quantity_on_hand,quantity_reserved,reorder_level,updated_at)
        VALUES(v_var4,v_loc,20,0,5,now());
    END IF;
    PERFORM * FROM inventory WHERE variant_id=v_var5 AND location_id=v_loc;
    IF NOT FOUND THEN
        INSERT INTO inventory(variant_id,location_id,quantity_on_hand,quantity_reserved,reorder_level,updated_at)
        VALUES(v_var5,v_loc,25,0,5,now());
    END IF;
    PERFORM * FROM inventory WHERE variant_id=v_var6 AND location_id=v_loc;
    IF NOT FOUND THEN
        INSERT INTO inventory(variant_id,location_id,quantity_on_hand,quantity_reserved,reorder_level,updated_at)
        VALUES(v_var6,v_loc,15,0,5,now());
    END IF;

    -- ===== 8. STOCK MOVEMENTS (initial inbound) =====
    INSERT INTO stock_movements(variant_id,location_id,movement_type,on_hand_delta,reserved_delta,reference_type,reference_id,idempotency_key,created_by,note)
    VALUES(v_var1,v_loc,'inbound',50,0,'initial',gen_random_uuid(),'init:v1',v_warehouse,'Nhập ban đầu');
    INSERT INTO stock_movements(variant_id,location_id,movement_type,on_hand_delta,reserved_delta,reference_type,reference_id,idempotency_key,created_by,note)
    VALUES(v_var2,v_loc,'inbound',40,0,'initial',gen_random_uuid(),'init:v2',v_warehouse,'Nhập ban đầu');
    INSERT INTO stock_movements(variant_id,location_id,movement_type,on_hand_delta,reserved_delta,reference_type,reference_id,idempotency_key,created_by,note)
    VALUES(v_var3,v_loc,'inbound',30,0,'initial',gen_random_uuid(),'init:v3',v_warehouse,'Nhập ban đầu');
    INSERT INTO stock_movements(variant_id,location_id,movement_type,on_hand_delta,reserved_delta,reference_type,reference_id,idempotency_key,created_by,note)
    VALUES(v_var4,v_loc,'inbound',20,0,'initial',gen_random_uuid(),'init:v4',v_warehouse,'Nhập ban đầu');
    INSERT INTO stock_movements(variant_id,location_id,movement_type,on_hand_delta,reserved_delta,reference_type,reference_id,idempotency_key,created_by,note)
    VALUES(v_var5,v_loc,'inbound',25,0,'initial',gen_random_uuid(),'init:v5',v_warehouse,'Nhập ban đầu');
    INSERT INTO stock_movements(variant_id,location_id,movement_type,on_hand_delta,reserved_delta,reference_type,reference_id,idempotency_key,created_by,note)
    VALUES(v_var6,v_loc,'inbound',15,0,'initial',gen_random_uuid(),'init:v6',v_warehouse,'Nhập ban đầu');
    ...
    IF v_order1 IS NULL THEN
        ...
        VALUES(gen_random_uuid(),'DH-001',v_customer1,'Nguyen Van A','0905555551','a@ettee.vn',
        '{"province":"TP.HCM","district":"Quan 1","ward":"Phu Nhuan","street_address":"123 Duong Ba"}',
        'web',gen_random_uuid(),'confirmed','cod','unpaid','VND',597000,0,30000,0,627000,0,
        now()-interval '2 days',now()-interval '2 days',now()-interval '1 day',now(),now())
        RETURNING id INTO v_order1;
        INSERT INTO order_status_history(id,order_id,from_status,status,order_version,changed_by,note,changed_at)
        VALUES(gen_random_uuid(),v_order1,'draft','draft',1,v_admin,'Tạo đơn',now());
        INSERT INTO order_status_history(id,order_id,from_status,status,order_version,changed_by,note,changed_at)
        VALUES(gen_random_uuid(),v_order1,'draft','confirmed',2,v_sales,'Xác nhận đơn',now());
    END IF;

    SELECT id INTO v_order2 FROM orders WHERE order_code='DH-002';
    IF v_order2 IS NULL THEN
        INSERT INTO orders(id,order_code,customer_id,customer_name,customer_phone,customer_email,shipping_address,source_platform,checkout_key,status,payment_method,payment_status,currency,subtotal,discount_total,shipping_fee,shipping_discount,total,version,placed_at,submitted_at,confirmed_at,updated_at,created_at)
        VALUES(gen_random_uuid(),'DH-002',v_customer2,'Tran Thi B','0905555552','b@ettee.vn',
        '{"province":"Ha Noi","district":"Ba Dinh","ward":"Phuc Xa","street_address":"456 Pho"}',
        'app',gen_random_uuid(),'picking','cod','unpaid','VND',897000,0,40000,0,937000,0,
        now()-interval '1 day',now()-interval '1 day',now(),now(),now())
        RETURNING id INTO v_order2;
        INSERT INTO order_status_history(id,order_id,from_status,status,order_version,changed_by,note,changed_at)
        VALUES(gen_random_uuid(),v_order2,'draft','draft',1,v_admin,'Tạo đơn',now());
        INSERT INTO order_status_history(id,order_id,from_status,status,order_version,changed_by,note,changed_at)
        VALUES(gen_random_uuid(),v_order2,'draft','pending_confirmation',2,v_admin,'Chờ xác nhận',now());
        INSERT INTO order_status_history(id,order_id,from_status,status,order_version,changed_by,note,changed_at)
        VALUES(gen_random_uuid(),v_order2,'pending_confirmation','confirmed',3,v_admin,'Xác nhận',now());
        INSERT INTO order_status_history(id,order_id,from_status,status,order_version,changed_by,note,changed_at)
        VALUES(gen_random_uuid(),v_order2,'confirmed','picking',4,v_warehouse,'Bắt đầu lấy hàng',now());
    END IF;

    -- ===== 10. ORDER ITEMS =====
    SELECT id INTO v_oi1 FROM order_items WHERE order_id=v_order1 AND variant_id=v_var1;
    IF v_oi1 IS NULL THEN
        INSERT INTO order_items(id,order_id,variant_id,product_name_snapshot,sku_snapshot,variant_snapshot,unit_price,quantity,discount_amount,created_at)
        VALUES(gen_random_uuid(),v_order1,v_var1,'Áo thun trắng','ATT-TR-S','{"size":"S","color":"Xanh"}',199000,2,0,now());
        INSERT INTO order_items(id,order_id,variant_id,product_name_snapshot,sku_snapshot,variant_snapshot,unit_price,quantity,discount_amount,created_at)
        VALUES(gen_random_uuid(),v_order1,v_var3,'Áo thun đỏ','ATT-TR-L','{"size":"L","color":"Đỏ"}',199000,1,0,now());
    END IF;

    SELECT id INTO v_oi2 FROM order_items WHERE order_id=v_order2 AND variant_id=v_var4;
    IF v_oi2 IS NULL THEN
        INSERT INTO order_items(id,order_id,variant_id,product_name_snapshot,sku_snapshot,variant_snapshot,unit_price,quantity,discount_amount,created_at)
        VALUES(gen_random_uuid(),v_order2,v_var4,'Quần jeans xanh','QJ-B-S','{"size":"S","color":"Xanh"}',399000,2,0,now());
        INSERT INTO order_items(id,order_id,variant_id,product_name_snapshot,sku_snapshot,variant_snapshot,unit_price,quantity,discount_amount,created_at)
        VALUES(gen_random_uuid(),v_order2,v_var5,'Quần jeans xanh','QJ-B-M','{"size":"M","color":"Xanh"}',399000,1,0,now());
    END IF;

    -- ===== 11. PAYMENTS =====
    INSERT INTO payments(id,order_id,method,provider,provider_account,amount,currency,status,idempotency_key,transaction_ref,paid_at,created_at,updated_at)
    VALUES(gen_random_uuid(),v_order1,'cod','cod','default',627000,'VND','pending',gen_random_uuid()::TEXT,NULL,now(),now(),now())
    ON CONFLICT DO NOTHING;
    INSERT INTO payments(id,order_id,method,provider,provider_account,amount,currency,status,idempotency_key,transaction_ref,paid_at,created_at,updated_at)
    VALUES(gen_random_uuid(),v_order2,'cod','cod','default',937000,'VND','pending',gen_random_uuid()::TEXT,NULL,now(),now(),now())
    ON CONFLICT DO NOTHING;
    ...
    IF v_ship1 IS NULL THEN
        ...
        VALUES(gen_random_uuid(),v_order1,'GHN','GHN-001-001','handed_over',v_shipper,v_warehouse,now()-interval '6 hours',now()-interval '1 hour',627000,NULL,now(),now(),0)
    ...
    IF v_ship2 IS NULL THEN
        INSERT INTO shipments(id,order_id,carrier,tracking_code,status,assigned_to,packed_by,packed_at,handed_over_at,cod_amount,delivery_proof_url,created_at,updated_at,version)
        VALUES(gen_random_uuid(),v_order2,'NINJAVAN','NV-002-001','in_transit',v_shipper,v_warehouse,now()-interval '3 hours',now(),937000,NULL,now(),now(),0)
    ...
    IF v_receipt IS NULL THEN
        ...
        VALUES(gen_random_uuid(),'NH-001','Nhap Kho VN','posted',v_warehouse,now()-interval '1 day')
        RETURNING id INTO v_receipt;
        INSERT INTO stock_receipt_items(id,receipt_id,variant_id,location_id,quantity,unit_cost,movement_id)
        VALUES(gen_random_uuid(),v_receipt,v_var1,v_loc,20,150000,NULL);
        INSERT INTO stock_receipt_items(id,receipt_id,variant_id,location_id,quantity,unit_cost,movement_id)
        VALUES(gen_random_uuid(),v_receipt,v_var4,v_loc,10,250000,NULL);
    END IF;
