...
public interface InventoryRepository extends JpaRepository<Inventory, InventoryId> {
    ...
    // Khóa dòng (SELECT ... FOR UPDATE) để tránh race-condition khi nhiều request cùng trừ/cộng tồn kho.
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("select i from Inventory i where i.id.variantId = :variantId and i.id.locationId = :locationId")
    Optional<Inventory> lockByVariantAndLocation(@Param("variantId") UUID variantId, @Param("locationId") UUID locationId);
