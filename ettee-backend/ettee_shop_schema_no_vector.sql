...
-- CREATE EXTENSION IF NOT EXISTS vector;       -- cần cài thêm extension pgvector riêng, không có sẵn trên Windows mặc định
...
/*
-- Đã tắt: liên quan bảng embedding (cần pgvector, chưa cài)
...
-- ALTER TABLE product_embeddings ADD CONSTRAINT fk_product_embedding_model FOREIGN KEY(model_version_id) REFERENCES model_versions(id);  -- Đã tắt: liên quan bảng embedding (cần pgvector, chưa cài)
-- ALTER TABLE user_embeddings ADD CONSTRAINT fk_user_embedding_model FOREIGN KEY(model_version_id) REFERENCES model_versions(id);  -- Đã tắt: liên quan bảng embedding (cần pgvector, chưa cài)
...
-- CREATE INDEX idx_product_embedding_version ON product_embeddings(model_version_id,modality);  -- Đã tắt: liên quan bảng embedding (cần pgvector, chưa cài)
...
/*
-- Đã tắt: liên quan bảng embedding (cần pgvector, chưa cài) (dùng vector_dims/vector_norm của pgvector)
...
-- COMMENT ON TABLE product_embeddings IS 'Version + modality; vector_dims được kiểm tra theo registry. Dùng item behavior embedding cùng space với query SASRec.';  -- Đã tắt: liên quan bảng embedding (cần pgvector, chưa cài)
