...
type ShippingException = {
  id: number; exceptionType: string; description: string; status: string;
  shipment?: { id: number; trackingCode?: string };
...
export default function ShippingExceptionsPage() {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!Number.isSafeInteger(shipmentId) || shipmentId <= 0 || !form.type.trim() || !form.description.trim()) { setError("Nhập ID kiện hợp lệ, loại và mô tả ngoại lệ."); return; }
  ...
  return <main className="min-h-screen bg-slate-950 p-6 text-slate-100 md:p-10"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <form onSubmit={create} className="grid gap-3 rounded-xl border border-slate-800 bg-slate-900 p-5 md:grid-cols-4">
      <input required type="number" min={1} value={form.shipmentId} onChange={e => setForm(v => ({ ...v, shipmentId:e.target.value }))} placeholder="ID kiện hàng" aria-label="ID kiện hàng" className="rounded border border-slate-700 bg-slate-950 px-3 py-2" />
