...
type Order = { id?: number; orderId?: number; orderCode?: string; customerName?: string; customerEmail?: string; phone?: string; shippingAddress?: string; status: string };
type Label = { orderId: number; orderCode?: string; receiver?: string; phone?: string; address?: string; codAmount?: number };
const orderId = (order: Order) => order.id ?? order.orderId;
export default function PackingPage() {
  ...
  const completePacking = async (id: number) => {
  ...
  const openLabel = async (id: number) => {
    setBusy(id); setError("");
