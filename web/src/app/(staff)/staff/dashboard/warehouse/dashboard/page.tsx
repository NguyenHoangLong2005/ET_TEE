...
type Inventory = { id: number; productName: string };
...
export default function WarehouseDashboardPage() {
  ...
  return <main className="min-h-screen bg-slate-950 px-5 py-8 text-slate-100 md:px-10"><div className="mx-auto max-w-6xl space-y-7">
    ...
    <section className="rounded-xl border border-slate-800 bg-slate-900 p-6"><h2 className="text-lg font-bold">Luồng kho</h2><p className="mt-2 text-sm text-slate-400">Đã xác nhận → Lấy hàng → Đóng gói → Bàn giao vận chuyển. Backend hiện dùng PICKING → PACKED cho cả thao tác hoàn tất picking và packing.</p><nav className="mt-5 flex flex-wrap gap-2">{[{name:"Tổng quan",href:"dashboard"},{name:"Đơn cần xử lý",href:"orders"},{name:"Nhập kho",href:"receiving"},{name:"Kiểm đếm/Kiểm kê",href:"stock-count"},{name:"Quản lý vị trí",href:"inventory"},{name:"Duyệt chênh lệch",href:"adjustments"},{name:"Giữ hàng",href:"reservations"},{name:"Lấy hàng",href:"picking"},{name:"Đóng gói",href:"packing"},{name:"Bàn giao",href:"shipments"},{name:"Đề xuất nhập thêm",href:"replenishment"}].map(item => <Link key={item.href} href={`/staff/dashboard/warehouse/${item.href}`} className="rounded border border-orange-500/40 px-3 py-2 text-sm text-orange-300 hover:bg-orange-500/10">{item.name} →</Link>)}</nav></section>
  </div></main>;
}
