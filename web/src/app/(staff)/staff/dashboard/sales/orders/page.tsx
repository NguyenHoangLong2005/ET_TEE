...
type Order = {
  id?: number; orderId?: number; orderCode?: string; customerName?: string; customerEmail?: string;
  phone?: string; shippingAddress?: string; status: string; paymentStatus?: string;
  total?: number; slaDeadline?: string;
};
type OrderNote = { id: number; content: string; createdAt?: string; createdBy?: number };
const getId = (order: Order) => order.id ?? order.orderId;
...
export default function SalesOrdersPage() {
  ...
  const verify = (order: Order) => {
    const id=getId(order); if (id == null) return;
  ...
  const reserve = (order: Order) => {
    const id=getId(order); if (id == null) return;
    const rawProduct=window.prompt("ID sản phẩm cần giữ:"); if (rawProduct === null) return;
    const rawQty=window.prompt("Số lượng cần giữ:"); if (rawQty === null) return;
    const productId=Number(rawProduct), quantity=Number(rawQty);
    if (!Number.isSafeInteger(productId) || productId <= 0 || !Number.isSafeInteger(quantity) || quantity <= 0) { setError("ID sản phẩm và số lượng phải là số nguyên dương."); return; }
    void perform(id, "reservations", "POST", { productId, quantity });
  };
  const addNote = (order: Order) => {
    ...
    // User identity must come from authenticated backend when RBAC is added.
